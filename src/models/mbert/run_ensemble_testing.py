...
# }
    ...
    new_args = copy.deepcopy(experiments_args_dict[experiments_args_dict.keys()[0]])
    ...
    # Now evaluate the ensemble on the swisstext_task1_test dataset
    predictions_dict = {}
    ...
    # Now we have the predictions of all models in the predictions_list, we do the ensemble prediction by majority voting
    predictions_df = pd.DataFrame()

    for test_sample in range(len(predictions_dict[list(predictions_dict.keys())[0]])):
        sample_predictions = []
        for model_name, model_predictions in predictions_dict.items():
            sample_predictions.append(model_predictions[test_sample])

        # Now we add the logits of each model for the test sample
        predictions = torch.add([sample_predictions])

        # We take the argmax of the predictions to get the ensemble prediction

        ensemble_prediction = torch.argmax(predictions, dim=1).item()

        # We add the ensemble prediction to the predictions_df
        predictions_df = predictions_df.append({'label': labels_list[test_sample], 'prediction': ensemble_prediction}, ignore_index=True)
